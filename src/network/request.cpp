...
)
{
    try
    {
        ...
        // Read size
        const uint64_t headerSize = co_await readNumber<uint64_t>(socket);
...
asio::awaitable<std::optional<Block>> requestHeaders(asio::ip::tcp::socket& socket,
                                                     const std::vector<BlockHeader>& commonAncestor)
{
    try
    {
        //TODO: make function
...
asio::awaitable<std::optional<std::vector<Tx>>> requestMempool(asio::ip::tcp::socket& socket)
{
    ...
    {
        // Read inv size
        const uint64_t invSize = co_await readNumber<uint64_t>(socket);
        ...
        // Tell missing size
        co_await writeNumber(socket, missing.size());
        ...
        {
            // Read size
            const uint64_t txSize = co_await readNumber<uint64_t>(socket);
